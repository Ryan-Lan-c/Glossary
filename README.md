...
- **Eventual Consistency(最終一致性)**— 分散式系統放棄「即時一致」,只保證「**若無新更新,所有副本最終收斂到一致**」,中間狀態短暫可見;CAP 下選 AP 的結果。詳見 [B4 分散式交易協定](./B4-persistence.md#distributed-tx)。
...
| B4 | [持久層與資料](./B4-persistence.md) | JPA、Repository、Entity、Blob / Lob、Mapper、MapStruct、Cache、Redis、TDE、索引(Clustered / Non-clustered)、分散式交易協定(2PC / 3PC / XA / JTA)、Outbox Pattern、Saga(跨系統 Rollback)、CQRS、Event Sourcing |
...
| F2 | [AI Agent 與 LLM 應用](./F2-ai-agent.md) | LLM / Token / Prompt / Context Window、TensorFlow / DL4J / ONNX / DJL、Tool Use、Hermes Agent、Browser Agent、網頁擷取(Crawl4AI / Firecrawl)、LSP、MCP、Spring AI / LangChain4j、RAG、Fine-tuning vs RAG |
...
| 版本 | 日期 | 變更摘要 |
| --- | --- | --- |
| **2.4.9** | **2026-07-17** | **F2 新增「LLM-ready 網頁擷取(Crawl4AI / Firecrawl)」條目**(新增 `<a id="web-extraction">`,置於 具體 Agent 框架與工具 段、Browser Agent 之後):RAG 資料進料端定位、傳統爬蟲兩大痛點(原始 HTML 雜訊 / JS 空殼)、無頭瀏覽器 + 自動轉 Markdown、Crawl4AI(開源自架)vs Firecrawl(SaaS)對照表、與 [Browser Agent](./F2-ai-agent.md#browser-agent)「操作 vs 擷取」區隔、Crawl4AI Python 範例、Java 端接 REST API 進 RAG pipeline;連 [RAG](./F2-ai-agent.md#rag) / [Spring AI](./F2-ai-agent.md#spring-ai) / [LangChain4j](./F2-ai-agent.md#langchain4j)。F2 目錄 + README F2 索引摘要補該條目。 |
| **2.4.8** | **2026-07-16** | **B4 新增「分散式交易協定(2PC / 3PC / XA / JTA)」條目**(新增 `<a id="distributed-tx">`,置於 進階模式 首則):2PC 兩階段 + blocking 痛點、3PC(PreCommit + timeout、實務罕用)、XA(X/Open DTP、TM/RM 介面)、JTA(`XAResource` / `XADataSource`、Narayana / Atomikos、Bitronix 於 Spring Boot 3.0 移除)、crash recovery / in-doubt transaction、強一致 vs 最終一致光譜;依主場原則 Saga 對照表加連結指向此條(不重複詳述),連 [Saga](./B4-persistence.md#saga) / [Outbox](./B4-persistence.md#outbox) / [Transaction Propagation](./B4-persistence.md#tx-propagation)。**Eventual Consistency 折入此條並於 README 通用詞彙補一行**(主場在此,跨領域概念不另開節)。<br>**B4 新增「索引(Index)」段與 Clustered / Non-clustered Index 條目**(新增 `<a id="clustered-index">`,置於 資料安全 與 進階模式 之間):clustered(資料實體排序、一表一個、InnoDB = PK)/ non-clustered(獨立結構、可多個、回表 key lookup)/ covering index 免回表、InnoDB 範例與口訣;底層結構連 [G1 B-Tree / B+Tree](./G1-data-structures.md#b-tree)。<br>**B4 Event Sourcing 條目從精簡補為完整**:補為什麼用(審計 / 時光回溯 / 配 CQRS / 驅動 Choreography Saga)、核心元件(Event Store / Aggregate / Snapshot / Projection)、帳戶事件流範例、代價(schema 演進 / upcasting / 最終一致)、何時用 / 不用;連 [CQRS](./A3-architecture.md#cqrs) / [Outbox](./B4-persistence.md#outbox) / [Saga](./B4-persistence.md#saga)。**B4 目錄新增「索引」段與「分散式交易協定」項、README B4 索引摘要補「索引、分散式交易協定」。** |
